...
/**
 * Register Credentials (with wallet address)
 */
...
export async function register(credentials: RegisterCredentials): Promise<AuthResult> {
  try {
    ...
    if (existingUser) {
      ...
      if (existingUser.walletAddress === credentials.walletAddress) {
    ...
    const user = await prisma.user.create({
      data: {
        ...
        displayName: credentials.displayName || credentials.username || `User ${Date.now()}`,
        // Password hash stored in separate table in production
      },
      select: {
        id: true,
        walletAddress: true,
